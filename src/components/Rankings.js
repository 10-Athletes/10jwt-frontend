    ...
    if (this.state.sport !== "") {
      this.state.sports.forEach((sport) => {
        if (sport.id === this.state.unsetID) {
          ...
          this.setState({
            sportID: this.state.sportMatches[0].id,
            sport: "",
            sportMatches: [],
            error: "",
          });
          ...
          if (
            ...
            if (officialCount === 1 && officialOrNot === "") {
              best = (
                <span className="d-none d-sm-block">
                  <br />
                  Best {sport.wordForPlayer} in the World
                </span>
              );
              if (sport.id === 268) {
                best = (
            ...
            participants.push(
              <tr style={{ fontWeight: fontWeight }} key={participant.id}>
                ...
                <td>
                  <a href={link}>{participant.name}</a>
                  {best}
                </td>
      ...
      if (sport.id === this.state.unsetID) {
        ...
        if (!unsetIncluded) {
          sportList.push(
            <ListGroup.Item
              action
              className="text-center sports-list-item"
              id={sport.id}
              key={`sport${sport.id}`}
              name={sport.name}
            >
      ...
      <div>
        <div className="my-5 mx-auto w-full">
                ...
                <div className="w-full">
                  <InputGroup className="mb-3" controlId="sport">
                    <Form.Control
                      ...
                      onBlur={() =>
                        setTimeout(() => this.hideSearchResults(), 1500)
                      }
                      required
                    />
                    <button
                      type="submit"
                      className="ml-3 py-3 px-5 bg-gray-200"
                  ...
                  <div
                    key="small-screen-official"
                    className="text-center official-or-not"
                  >
                    <Form.Check
                      inline
                      type="radio"
          ...
          <div>
            <div className="d-none d-lg-block">
              <Form className="mx-3 mt-5">
                ...
                <div
                  ...
                  className="mb-5 official-or-not"
                >
                  <Form.Check
                    type="radio"
                    name="official"
                ...
                <button
                  type="submit"
                  className="py-3 px-5 bg-gray-200 w-100"
                  onClick={this.handleSubmit}
                >
                  <b>View {nextSport}</b>
                </button>{" "}
            ...
            <div className="w-full lg:w-1/3 lg:mx-auto text-left">
              <h1 style={{ textAlign: "center" }}>
                {unofficial}
                {sportName} World Rankings
              </h1>
              <table className="rankings-list w-full" bordered striped hover>
                <thead>
                  <tr>
                    <th className="px-3" style={{ width: "1px" }}>
                      Rank
                    </th>
                    <th>Name</th>
                    <th className="px-3" style={{ width: "1px" }}>
